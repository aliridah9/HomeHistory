...
const properties_service_1 = require("../../src/modules/properties/properties.service");
const prisma_service_1 = require("../../src/modules/database/prisma.service");
const geolocation_service_1 = require("../../src/modules/properties/services/geolocation.service");
describe('PropertiesService', () => {
    ...
    beforeEach(async () => {
        ...
        geolocationService = module.get(geolocation_service_1.GeolocationService);
        // Reset mocks
        jest.clearAllMocks();
    });
    describe('createProperty', () => {
        ...
        it('should create a property successfully', async () => {
            ...
            expect(mockGeolocationService.geocodeAddress).toHaveBeenCalledWith(`${createPropertyDto.address}, ${createPropertyDto.city}, ${createPropertyDto.state} ${createPropertyDto.zipCode}`);
    ...
    describe('getPropertyById', () => {
        ...
        it('should throw NotFoundException if property does not exist', async () => {
            ...
            await expect(service.getPropertyById('test-user-id', 'non-existent-id')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw ForbiddenException if user does not own property', async () => {
            ...
            await expect(service.getPropertyById('test-user-id', 'test-property-id')).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe('updateProperty', () => {
        ...
        it('should throw NotFoundException if property does not exist', async () => {
            mockPrismaService.property.findUnique.mockResolvedValue(null);
            await expect(service.updateProperty('test-user-id', 'non-existent-id', updateDto)).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw ForbiddenException if user does not own property', async () => {
            const otherUserProperty = { ...mockProperty, userId: 'other-user-id' };
            mockPrismaService.property.findUnique.mockResolvedValue(otherUserProperty);
            await expect(service.updateProperty('test-user-id', 'test-property-id', updateDto)).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe('deleteProperty', () => {
        ...
        it('should throw NotFoundException if property does not exist', async () => {
            mockPrismaService.property.findUnique.mockResolvedValue(null);
            await expect(service.deleteProperty('test-user-id', 'non-existent-id')).rejects.toThrow(common_1.NotFoundException);
        });
    });
