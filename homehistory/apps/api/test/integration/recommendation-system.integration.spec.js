...
describe('HomeHistory Recommendation System Integration (e2e)', () => {
    ...
    let sourceProperty;
    let similarProperty1;
    let similarProperty2;
    let differentProperty;
    beforeAll(async () => {
        ...
        recommendationService = moduleFixture.get(recommendation_service_1.RecommendationService);
        // Create test users
        testUser = await (0, setup_1.createTestUser)({ role: client_1.UserRole.USER });
        adminUser = await (0, setup_1.createTestUser)({ role: client_1.UserRole.ADMIN });
        userToken = (0, setup_1.generateTestToken)(testUser.id);
        adminToken = (0, setup_1.generateTestToken)(adminUser.id);
        // Create test properties with similar characteristics
        sourceProperty = await (0, setup_1.createTestProperty)(testUser.id, {
            address: '123 Main St',
            city: 'Austin',
            state: 'TX',
            zipCode: '78701',
            propertyType: client_1.PropertyType.SINGLE_FAMILY,
            bedrooms: 3,
            bathrooms: 2,
            squareFeet: 2000,
            yearBuilt: 2015,
        ...
        similarProperty1 = await (0, setup_1.createTestProperty)(testUser.id, {
            address: '456 Oak Ave',
            city: 'Austin',
            state: 'TX',
            zipCode: '78701',
        ...
        similarProperty2 = await (0, setup_1.createTestProperty)(testUser.id, {
            address: '789 Pine Rd',
            city: 'Austin',
            state: 'TX',
            zipCode: '78704',
        ...
        differentProperty = await (0, setup_1.createTestProperty)(testUser.id, {
        ...
        jest.spyOn(recommendationService, 'generatePropertyEmbedding').mockImplementation(async (property) => {
            ...
            const mockEmbedding = Array(1536).fill(0).map((_, i) => {
                ...
                if (property.bedrooms === 3)
                    value += 0.1;
                if (property.price && property.price >= 400000 && property.price <= 500000)
                    value += 0.1;
    ...
    describe('GET /api/properties/:id/similar', () => {
        ...
        it('should exclude the source property from recommendations', async () => {
            const response = await request(app.getHttpServer())
                ...
                .set('Authorization', `Bearer ${userToken}`)
                .expect(200);
            const recommendedIds = response.body.recommendations.map((rec) => rec.property.id);
    ...
    describe('Recommendation Algorithm Validation', () => {
        it('should calculate similarity scores within valid ranges', async () => {
            const similarity = await recommendationService.calculateSimilarity(sourceProperty, similarProperty1);
        ...
        it('should rank more similar properties higher', async () => {
            const recommendations = await recommendationService.findSimilarProperties(sourceProperty.id, { limit: 10 });
            if (recommendations.length > 1) {
                // Recommendations should be sorted by similarity score (descending)
                for (let i = 1; i < recommendations.length; i++) {
                    expect(recommendations[i - 1].similarityScore).toBeGreaterThanOrEqual(recommendations[i].similarityScore);
                }
            }
        });
        it('should diversify results to avoid too many similar properties', async () => {
            const recommendations = await recommendationService.findSimilarProperties(sourceProperty.id, { limit: 10, diversityWeight: 0.5 });
        ...
        it('should exclude properties outside distance range', async () => {
            const recommendations = await recommendationService.findSimilarProperties(sourceProperty.id, { maxDistance: 5 } // 5km radius
            );
        ...
        it('should exclude properties outside price range', async () => {
            const recommendations = await recommendationService.findSimilarProperties(sourceProperty.id, { priceRangePercent: 0.05 } // ±5%
            );
            const sourcePrice = sourceProperty.price;
    ...
    describe('Error Handling', () => {
        it('should handle missing property embeddings gracefully', async () => {
            ...
            const newProperty = await (0, setup_1.createTestProperty)(testUser.id, {
...
//# sourceMappingURL=recommendation-system.integration.spec.js.map