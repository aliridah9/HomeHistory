...
export const authApi = {
  ...
  updateProfile: (data: Partial<any>) => api.put<ApiResponse<any>>('/auth/profile', data),
};

// Landing data helpers (fetch with credentials, graceful fallbacks)
export const API_BASE_URL_LANDING =
...
export const propertiesApi = {
  ...
  getProperty: (id: string) => api.get<ApiResponse<any>>(`/properties/${id}`),

  createProperty: (data: any) => api.post<ApiResponse<any>>('/properties', data),

  updateProperty: (id: string, data: Partial<any>) =>
    api.put<ApiResponse<any>>(`/properties/${id}`, data),

  deleteProperty: (id: string) => api.delete<ApiResponse<null>>(`/properties/${id}`),

  getPropertyScore: (id: string) => api.get<ApiResponse<any>>(`/properties/${id}/score`),
...
export const searchApi = {
  ...
  saveSearch: (searchData: { query: string; filters: any; name?: string }) =>
    api.post<ApiResponse<any>>('/search/saved', searchData),

  getSavedSearches: () => api.get<ApiResponse<any[]>>('/search/saved'),

  deleteSavedSearch: (id: string) => api.delete<ApiResponse<null>>(`/search/saved/${id}`),
};
...
export const aiApi = {
  ...
  getUsageMetrics: () => api.get<ApiResponse<any>>('/ai/usage'),
};

// Admin AI API
export const adminAiApi = {
  getDashboard: () => api.get<ApiResponse<any>>('/admin/ai/dashboard'),

  getMetrics: (params?: { timeframe?: string; service?: string }) =>
    api.get<ApiResponse<any>>('/admin/ai/metrics', { params }),

  getAlerts: (params?: { resolved?: boolean; severity?: string; limit?: number }) =>
    api.get<ApiResponse<any>>('/admin/ai/alerts', { params }),

  resolveAlert: (alertId: string) =>
    api.put<ApiResponse<any>>(`/admin/ai/alerts/${alertId}/resolve`),

  getCacheAnalytics: () => api.get<ApiResponse<any>>('/admin/ai/cache/analytics'),

  startCacheWarming: (data: { propertyIds?: string[]; services: string[]; priority?: string }) =>
    api.post<ApiResponse<any>>('/admin/ai/cache/warm', data),

  getCacheWarmingJob: (jobId: string) => api.get<ApiResponse<any>>(`/admin/ai/cache/jobs/${jobId}`),
  ...
  startBulkEmbeddingUpdate: (data: { propertyIds?: string[]; filters?: any; batchSize?: number }) =>
  ...
  getServiceHealth: () => api.get<ApiResponse<any>>('/admin/ai/service-health'),

  resetMetrics: () => api.post<ApiResponse<any>>('/admin/ai/maintenance/reset-metrics'),

  getConfiguration: () => api.get<ApiResponse<any>>('/admin/ai/configuration'),
};
...
export const usersApi = {
  ...
  getUser: (id: string) => api.get<ApiResponse<any>>(`/users/${id}`),

  createUser: (data: any) => api.post<ApiResponse<any>>('/users', data),

  updateUser: (id: string, data: Partial<any>) => api.put<ApiResponse<any>>(`/users/${id}`, data),

  deleteUser: (id: string) => api.delete<ApiResponse<null>>(`/users/${id}`),

  getUserActivity: (id: string) => api.get<ApiResponse<any[]>>(`/users/${id}/activity`),
...
export const documentsApi = {
  ...
  getDocument: (id: string) => api.get<ApiResponse<any>>(`/documents/${id}`),
  ...
  deleteDocument: (id: string) => api.delete<ApiResponse<null>>(`/documents/${id}`),

  getDocuments: (params?: { propertyId?: string; type?: string; page?: number; limit?: number }) =>
...
export const maintenanceApi = {
  ...
  createMaintenanceRecord: (data: any) => api.post<ApiResponse<any>>('/maintenance', data),

  updateMaintenanceRecord: (id: string, data: Partial<any>) =>
    api.put<ApiResponse<any>>(`/maintenance/${id}`, data),

  deleteMaintenanceRecord: (id: string) => api.delete<ApiResponse<null>>(`/maintenance/${id}`),
...
export const notificationsApi = {
  ...
  deleteNotification: (id: string) => api.delete<ApiResponse<null>>(`/notifications/${id}`),
...
export const healthApi = {
  getHealth: () => api.get<ApiResponse<any>>('/health'),

  getAIHealth: () => api.get<ApiResponse<any>>('/health/ai'),

  getDetailedAIHealth: () => api.get<ApiResponse<any>>('/health/ai/detailed'),

  getAIHealthMetrics: () => api.get<ApiResponse<any>>('/health/ai/metrics'),
};
